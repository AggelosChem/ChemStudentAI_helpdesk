...
# --- TAB 3: ADMIN DASHBOARD (SQL VERSION) ---
with tab3:
    ...
    if pwd == "admin123":
        ...
        st.markdown("---")
        st.write("### 🗂️ Διαχείριση (Live DB)")
        
        # Data Editor
        # Δείχνουμε μόνο τα ενεργά για ευκολία, ή όλα με checkbox
        ...
        edited_data = st.data_editor(
            ...
            column_config={
                ...
                "id": st.column_config.TextColumn(disabled=True) # Να μην αλλάζουν το ID!
            },
            hide_index=True,
            use_container_width=True
        )

        if st.button("💾 Αποθήκευση Αλλαγών στη Βάση"):
            try:
                # Καλλούμε τη συνάρτηση Update
                update_tickets_batch(edited_data)
                st.success("✅ Η βάση δεδομένων ενημερώθηκε!")
                time.sleep(1)
                st.rerun()
